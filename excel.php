...
$result = mysqli_query($connect, "SELECT * FROM factura");
if($result->num_rows > 0 ){
    ...
    $tituloReporte = "Factura ".date('d-m-y');
    $titulosColumnas = array('EMPRESA', 'Destino', 'Origen', 'Peso', 'TASA', 'Fuel', 'Reembolso', 'Final', 'Fecha');
    ...
    while ($fila = $result->fetch_array()) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D'.$i, $fila['peso'].'Kg')
            ->setCellValue('E'.$i, $fila['tasaPeso'])
            ->setCellValue('F'.$i, $fila['fuel'])
            ->setCellValue('G'.$i, $fila['reembolso'])
            ->setCellValue('H'.$i, $fila['precioFinal'])
            ->setCellValue('I'.$i, $fila['Fecha']);
        $i++;
        $objPHPExcel->setActiveSheetIndex(0)
    ...
    $objPHPExcel->getActiveSheet()->setSharedStyle($estiloInformacion, "A4:I".$i);

    $objPHPExcel->getActiveSheet()->getStyle('E4:H'.$i)->getNumberFormat()->applyFromArray(
        array(
